/**
 * @file multipregame.h
 * @brief Header file for the MultiPregame class, which implements the pre-game
 ...
 *
 * @details The MultiPregame class manages the pre-game setup phase where
 * players join the lobby, select their roles, and prepare for the game to
 * start. It handles both host and client functionality, manages player
 * connections, role assignments, and transitions to the game.
 *
 * @author Group 9
 */
class MultiPregame : public QWidget {
 ...
 public:
   ...
   *
   * @details Initializes the pre-game lobby for a host player who is creating a
   * new game. Sets up the server to accept client connections and manages the
   * player list.
   *
   ...
   * @author Group 9
   */
  MultiPregame(QWebSocketServer* server, const QString& username,
               QWidget* parent = nullptr);

  /**
   * @brief Constructor for the MultiPregame class when acting as a client.
   *
   * @details Initializes the pre-game lobby for a client player who is joining
   * an existing game. Sets up the connection to the host server and prepares
   * the player for role selection.
   *
   ...
   * @author Group 9
   */
  MultiPregame(QWebSocket* socket, const QString& username,
               QWidget* parent = nullptr);
   ...
   *
   * @details Removes all UI elements from the widget and prepares it for
   * rebuilding or transition to another state.
   *
   * @author Group 9
   */
  void clearUI();
 ...
 public slots:
   ...
   *
   * @details Initiates the game when all players are ready and roles are
   * assigned. Sends game start notification to all clients and transitions to
   * the game board. Only the host can trigger this action.
   *
   * @author Group 9
   */
  void startGame();
 ...
 private:
   ...
   * @author Group 9
   */
  void gameStarted(bool isHost, QWebSocketServer* server,
